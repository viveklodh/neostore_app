...
class _SignUpState extends State<SignUp> {
  ...
  Widget firstName() {
    return Padding(
      padding:
          const EdgeInsets.only(left: 15.0, right: 15.0, top: 8, bottom: 0),
      child: TextFormField(
        ...
        child: FlatButton(
          onPressed: () async {
            if (formKey.currentState.validate()) {
              ...
              try {
                var response = await NeoStoreNetwork()
                    .getRegisterResponse(registerRequestModel);
                if (response.statusCode == 200) {
                  var registerResponse =
                      RegisterModel.fromJson(json.decode(response.data));
                ...
                scaffoldKey.currentState.showSnackBar(SnackBar(
                  content: Text(errorResponse.userMsg),
                ));
